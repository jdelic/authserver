...
* ``--yes``/``-y`` is the standardized confirmation bypass flag.
...
meaning and short flag mapping. This standard also applies to aligning existing
commands in the current codebase (for example replacing ``--force`` usage with
``--yes`` in legacy command modules).
...
* Whether some multi-object ``edit`` operations should require explicit
  ``--yes`` even when only one row is currently matched by selector.
