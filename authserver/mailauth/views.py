...
class UserLoginAPIView(JWTViewHelperMixin, View):
    ...
    def _parse_basic_auth(self, request: HttpRequest) -> "_AuthRequest | None":
    ...
    def _parse_request(self, request: HttpRequest) -> _AuthRequest:
        ...
        if request.content_type == "application/json":
            ...
            scopes = request.POST["scopes"].split(",") if "scopes" in request.POST else []
    ...
    def post(self, request: HttpRequest) -> HttpResponse:
        ...
        userdesc = self._parse_basic_auth(request)
        if userdesc is None:
            try:
        ...
        if user is None:
            ...
            jwtstr = self._create_jwt(
                claim={
                    "sub": userdesc.username,
                    "canonical_username": "%s@%s" % (
                        user.delivery_mailbox.mailprefix,
                        user.delivery_mailbox.domain.name,
                    ),
