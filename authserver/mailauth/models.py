    ...
    def find_parent_domain(self, fqdn: str, require_jwt_subdomains_set: bool = True,
        ...
        for domainstr in [".".join(parts[r:]) for r in range(0, len(parts))]:
            try:
                ...
                if req_domain is None or (req_domain.jwtkey == "" and require_jwt_key):
                    req_domain = None
                    continue

                if (req_domain.jwtkey is not None and req_domain.jwtkey != "") or not require_jwt_key:
...
class Domain(models.Model):
    ...
    redirect_to = models.CharField(
        verbose_name="Redirect all mail to domain or email",
        max_length=255,
        help_text="Put a domain name here to redirect all email addresses to that domain (xyz@domain.com becomes "
                  "xyz@redirectdomain.com) or put an email address here to redirect all email to that one address "
